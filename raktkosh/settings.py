...
GEOPY_USER_AGENT = config('GEOPY_USER_AGENT', default='raktkosh-app')

# Render (and most PaaS hosts) terminate TLS at a proxy and forward plain HTTP
# to the app; this tells Django to trust that header when deciding if a request
# is secure. Harmless locally, where the header is simply absent.
...
if not DEBUG:
    ...
    SECURE_HSTS_SECONDS = 60 * 60 * 24 * 7  # 1 week to start; raise once confident HTTPS is solid
...
WSGI_APPLICATION = 'raktkosh.wsgi.application'
...
# ManifestStaticFilesStorage requires `collectstatic` to have run (it looks up a
# hashed-filename manifest), which local `runserver`/`test` never do. Use it only
# in production, where the deploy step always runs collectstatic first.
if DEBUG:
    STORAGES = {
